...
**为什么要这样?** → 把句子拆成Token(语言片段),就像把面包切片,AI才能一片片识别处理
...
**为什么要这样?** → 把Token转成数字ID,让AI能识别(AI的大脑只认数字,不认文字)
...
**为什么要这样?** → 把Token的"意思"编码成向量(一串数字),相似意思的Token向量也相似,AI就能理解语义关系了
...
**为什么要这样?** → 通过向量相似度匹配,从训练时学到的3000亿本书记忆里激活相关知识,就像闻到味道就想起往事
...
**为什么要这样?** → AI不是一次性想好整句话,而是根据已输出的内容,逐个预测下一个最可能的Token,就像你讲故事边说边想
